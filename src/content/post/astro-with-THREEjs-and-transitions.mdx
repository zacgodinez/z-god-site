...
You have probably seen with an eyeball or two 👀, the demo of Astro websites using native browser [page transitions](https://docs.astro.build/en/guides/view-transitions/).
...
Astro view transitions give us a simple fade as a default, transitions can be customized [customized](https://docs.astro.build/en/guides/view-transitions/) like this.
...
With out two astro pages setup with transitions let's add THREE.js to the pages.
...
Now we will reference is file from our `entry.ts` file.
...
The astro:before-swap and astro:page-load events are crucial for managing the lifecycle of your Three.js scene in an Astro context.
WebGL context


it’s used to clean up the existing Three.js scene, ensuring that any previously rendered resources (e.g., memory-intensive objects or animations) are properly disposed of.
...
As you can see here the three.js example works seemless with the page transitions.
